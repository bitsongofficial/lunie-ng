  ...
  <q-dialog ref="dialogRef" @hide="onDialogHide">
    <q-card class="body column items-center">
      <div class="dialog-header row items-center justify-between full-width">
        <h2 class="title text-body-large text-white q-my-none" v-if="!error && ! success">Burn</h2>
      ...
      <template v-if="!error">
        <q-form class="col column items-center fit" @submit="onSubmit" v-if="!success">
          <div class="field-block column full-width">
            <label class="field-label text-uppercase text-half-transparent-white text-h6 text-weight-medium">Amount to burn</label>
            ...
            >
              <template v-slot:append>
                ...
                <q-btn @click="request.amount = availableCoins.toString()" class="max-btn btn-super-extra-small text-body3" rounded unelevated color="primary" text-color="white" padding="4px 7px 3px">
                  MAX
                </q-btn>
              </template>
            </q-input>

            <p class="text-body2 text-half-transparent-white text-right q-px-sm q-mt-sm q-mb-none">Available to burn: {{ availableCoins }} <span class="text-uppercase">{{ fantoken.metaData?.display }}</span></p>
          </div>
        ...
        <div class="success col column fit" v-else>
          ...
          <p class="text-h4 text-half-transparent-white text-center">You have successfully burned your <span class="text-uppercase">{{ fantoken.metaData?.display }}</span>s.</p>
        </div>
      </template>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const onSubmit = async () => {
      try {
        const requestToSign = {
          type: MessageTypes.BURN_FANTOKEN,
          message: {
            ...request,
